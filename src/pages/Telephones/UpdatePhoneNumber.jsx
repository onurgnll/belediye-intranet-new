...
function UpdatePhoneNumber({ handleClose, open, getPhones, mudurlukler, setPage , phoneNumber}) {
    ...
    useEffect(() => {
        if(open){
            setName(phoneNumber.description || phoneNumber.name)
            setTelephone(phoneNumber.phone)
            setDepartman(phoneNumber.departmentID)
        }
    }, [open]);
    ...
    const handleSubmit = async (event) => {
        ...
        const resp = await requestWithAuth("put", "/admin/update-phonenumber/", phoneNumber.id, "", { name, departman, telephone })
            ...
            <div style={{ width: "35rem" }} className='p-5'>
                <form onSubmit={handleSubmit}>
                    ...
                    <FormControl className='mt-2' fullWidth>
                        ...
                        <Select
                            defaultValue={phoneNumber?.departmentID}
